...
export default {
  ...
  subscriptions: {
    setup1: function ({ dispatch, history }) {
      history.listen(function ({ pathname }) {
        const match = pathToRegexp('/itemBYGN/:type').exec(pathname);
        if (match) {
          dispatch({
            type: 'init',
            payload: {
  ...
  effects: {
    ...
    fetch: function* ({ payload: { type } }, { select, call, put }) {
      ...
      const { list: oList, page: oPage, size: oSize } = yield select(state => state['itemBYGN']);
      const { data: { list, params = {} } } = yield call(GET, `/api/itemBYGN`, {
        mark: type,
        page: Number(oPage) + 1,
        size: oSize,
      });
