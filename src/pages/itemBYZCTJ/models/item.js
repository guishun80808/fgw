...
export default {
  ...
  effects: {
    ...
    fetch: function* ({ payload }, { select, call, put }) {
      ...
      const { list: oList, page: oPage, size: oSize } = yield select(state => state['itemBYZCTJ']);
      const { data: { list, params = {} } } = yield call(GET, `/api/itemBYZCTJ`, {
        page: Number(oPage) + 1,
        size: oSize,
      });
